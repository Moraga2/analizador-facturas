...
# Lógica de la PILA (Antes bombilla)
estado_pila = "off"
...
st.markdown(f"""
    ...
    /* TÍTULO (CON EL DESPLAZAMIENTO QUE PEDISTE) */
    h1 {{
    ...
    .pila {{
        font-size: 40px;
        margin-bottom: -10px; /* Ajustado para que quede sobre la cabeza del robot */
        z-index: 102;
    ...
    /* ESTADOS DE LA PILA */
    .pila-on {{ filter: drop-shadow(0 0 20px #00FF00) brightness(1.2); opacity: 1; }}
    .pila-off {{ filter: grayscale(100%) brightness(0.3); opacity: 0.6; }}

    </style>
    """, unsafe_allow_html=True)

# --- FOOTER (ROBOT) ---
clase_css_pila = f"pila-{estado_pila}"
st.markdown(f"""
    <div class="footer-container">
        <div class="worker-wrapper">
            <div class="pila {clase_css_pila}">🔋</div>
            <div class="robot">🤖</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        if uploaded_file is not None:
            if uploaded_file != st.session_state.ultimo_archivo:
                ...
                # ROBOT VOLADOR (Animación de carga)
